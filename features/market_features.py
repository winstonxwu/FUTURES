...
class MarketFeatureBuilder:
    ...
    def build_features(
        self, ticker: str, bars: List[PriceBar], current_bar: PriceBar
    ) -> Dict[str, Any]:
        """
        ...
        current_return = (
            (current_bar.close - bars[-2].close) / bars[-2].close
            if len(bars) >= 2
            else 0
        )
        return_zscore = (
            (current_return - mean_return) / std_return if std_return > 0 else 0
        )
        ...
        volume_ratio = (
            current_bar.volume / avg_volume_30d if avg_volume_30d > 0 else 1.0
        )
