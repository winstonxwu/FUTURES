...
class TextFeatureBuilder:
    ...
    def build_features(
        ...
        ticker_events = [
            ...
            and e.published_at <= current_ts
            and e.published_at >= cutoff_ts
        ]
        ...
        sentiment_weighted = self._calculate_weighted_sentiment(
            ticker_events, current_ts
        )

        # Count events in last hour
        one_hour_ago = current_ts - timedelta(hours=1)
        event_count_1h = len(
            [e for e in ticker_events if e.published_at >= one_hour_ago]
        )
        ...
        recent_sentiment = (
            self._calculate_weighted_sentiment(recent_events, current_ts)
            if recent_events
            else 0
        )
        older_sentiment = (
            self._calculate_weighted_sentiment(older_events, six_hours_ago)
            if older_events
            else 0
        )
    ...
    def _calculate_weighted_sentiment(
        self, events: List[TextEvent], reference_ts: datetime
    ) -> float:
        """
    ...
    def _extract_event_tags(self, events: List[TextEvent]) -> Dict[str, float]:
        ...
        for event in events:
            ...
            if "guidance" in text and any(
                w in text for w in ["raise", "increase", "upgrade"]
            ):
                tags["guidance_up"] += 1
            if "guidance" in text and any(
                w in text for w in ["lower", "decrease", "downgrade"]
            ):
                tags["guidance_down"] += 1
            if (
                any(w in text for w in ["capex", "capital expenditure", "investment"])
            ...
            if any(
                w in text
                for w in ["ceo", "cfo", "executive", "resignation", "appointment"]
            ):
                tags["exec_change"] += 1
