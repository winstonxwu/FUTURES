...
class ModelTrainer:
    ...
    def prepare_training_data(
        ...
        feature_cols = [
            'sentiment_weighted', 'event_count_1h', 'sentiment_delta',
            'return_zscore', 'rsi', 'atr', 'volume_ratio', 'spread_bps'
        ]
    ...
    def train_all_models(
        ...
        # Evaluate P_up
        y_pred_up = np.array([
            self.p_up_model.predict(
                self._array_to_features(X_test[i])
            )
