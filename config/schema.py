...
class UniverseConfig(BaseModel):
    ...
    min_adv_usd: float = Field(5_000_000, description="Minimum average daily volume in USD")


class RiskConfig(BaseModel):
    """Risk management configuration"""
    kelly_scale: float = Field(0.5, ge=0, le=1, description="Kelly criterion scaling factor")
    max_total_exposure: float = Field(0.30, ge=0, le=1, description="Maximum total portfolio exposure")
    max_per_name: float = Field(0.05, ge=0, le=1, description="Maximum exposure per ticker")
    sector_cap: float = Field(0.15, ge=0, le=1, description="Maximum exposure per sector")
...
class ScoringConfig(BaseModel):
    ...
    z_threshold: float = Field(1.5, description="Z-score threshold for extension dampener")
    alpha_ext: float = Field(0.4, ge=0, description="Extension dampener alpha parameter")
    rsi_overbought: int = Field(75, ge=50, le=100, description="RSI overbought threshold")
    volume_spike_mult: float = Field(3.0, ge=1, description="Volume spike multiplier threshold")
    enter_threshold: float = Field(0.60, ge=0, le=1, description="Minimum score to enter position")
    kneejerk_cut: float = Field(0.60, ge=0, le=1, description="P_drop threshold for knee-jerk exit")
...
class TraderConfig(BaseModel):
    ...
    news_sources: List[str] = Field(
        default=["sec_edgar", "company_ir_rss", "businesswire", "prnewswire", "regulatory_feeds"],
        description="Whitelisted news sources"
    )
    sim_latency_seconds: int = Field(600, ge=0, description="Simulated latency in seconds (10 min default)")
...
DEFAULT_CONFIG = {
    ...
    "news_sources": ["sec_edgar", "company_ir_rss", "businesswire", "prnewswire", "regulatory_feeds"],
